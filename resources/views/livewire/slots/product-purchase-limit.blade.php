    ...
    <div class="flex-col px-4 py-5 space-y-4 bg-white rounded-md sm:p-6">
        <header class="flex items-center justify-between">
            <h3 class="text-lg font-medium leading-6 text-gray-900">
                {{ __('lunarphp-purchase-limit::slots.product.heading') }}
            </h3>
        </header>

        <div class="flex items-center justify-between pt-4 border-t">
            <div>
                <strong>{{ __('lunarphp-purchase-limit::slots.product.customer_groups.title') }}</strong>
                <p class="text-xs text-gray-600">
                    {{ __('lunarphp-purchase-limit::slots.product.customer_groups.strapline') }}
                </p>
            </div>
            <x-hub::button :disabled="count($customerGroupLimits) >= count($this->customerGroups) + 1" wire:click.prevent="addCustomerGroupLimit" theme="gray" size="sm" type="button">
                {{ __('lunarphp-purchase-limit::slots.product.customer_groups.add_group_btn') }}
            </x-hub::button>
        </div>
                        ...
                        <div class="flex items-center">
                            <div class="grid grid-cols-4 gap-4 grow">
                                ...
                                <div class="flex flex-col space-y-2">
                                    <label class="block text-sm font-medium text-slate-700">{{ __('lunarphp-purchase-limit::global.customer_group') }}</label>
                                    <x-hub::input.select
                                        wire:model='customerGroupLimits.{{ $index }}.customer_group_id'>
                                        <option value="*">{{ __('lunarphp-purchase-limit::global.any') }}</option>
                                ...
                                <div class="flex flex-col space-y-2">
                                    <label class="block text-sm font-medium text-slate-700">{{ __('lunarphp-purchase-limit::global.max_quantity') }}</label>
                                    <x-hub::input.text
                                ...
                                <div class="flex flex-col space-y-2">
                                    <label class="block text-sm font-medium text-slate-700">{{ __('lunarphp-purchase-limit::global.max_total') }}</label>
                                    <x-hub::input.text
                                ...
                                <div class="flex flex-col space-y-2">
                                    <label class="block text-sm font-medium text-slate-700">{{ __('lunarphp-purchase-limit::global.time_period') }}</label>
